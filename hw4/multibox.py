...
def imgGenerator(batchSize, train):
    ...
    Y = np.zeros(shape=(batchSize, 1420, 5))
    while True:
        if train:
        ...
        for i in range(batchSize):
            ...
            if train:
                X[i] = processImage(os.path.join(trainFolder, imgInfo[0]))
            else:
                X[i] = processImage(os.path.join(validationFolder, imgInfo[0]))
...
def bounding_box_prediction_layers(inputs, bboxes_per_cell, batch_size):
    ...
    endpoints['branch88_confs'] = Conv2D(bboxes_per_cell, (1, 1), strides=1, padding='same')(branch88)
    ...
    endpoints['branch66_confs'] = Conv2D(bboxes_per_cell, (1, 1), strides=1, padding='same')(branch66)
    ...
    endpoints['branch44_confs'] = Conv2D(bboxes_per_cell, (1, 1), strides=1, padding='same')(branch44)
    ...
    endpoints['branch33_confs'] = Conv2D(bboxes_per_cell, (1, 1), strides=1, padding='same')(branch33)
    ...
    endpoints['branch22_confs'] = Conv2D(bboxes_per_cell, (1, 1), strides=1, padding='same')(branch22)
    ...
    endpoints['branch11_confs'] = Conv2D(1, (1, 1), strides=1, padding='same')(branch11)
...
def loc_loss(ground_box, pred_box):
    return 0.5 * K.sum(K.square(ground_box - pred_box))

def conf_loss(conf, conf_sum):
    return -conf_sum + K.log(1-conf) - K.log(conf)
...
def multibox_loss(y_true, y_pred):
    ...
    for b in range(batchSize):
        ...
        conf_sum = K.sum(K.log(1 - confs_gt))
        conf_loss = -conf_sum + K.log(1-confs_gt) - K.log(confs_gt)

        loc_loss = 0.5 * K.sum(K.square(batch_gt - preds_gt), axis=1)
